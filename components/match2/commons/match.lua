...
local ErrorUtil = require('Module:Error/Util')
...
function Match.storeMatchGroup(matchRecords, options)
	...
	local preparedMatchRecords, errors_ = ErrorUtil.mapTry(matchRecords, function(matchRecord)
	...
	if options.storeMatch2 then
		local _, errors = ErrorUtil.mapTry(matchRecordsCopy, function(matchRecord)
			local records
			if LegacyMatch then
...
function Match.summarizeErrors(errors, messagePart)
	...
	local message = table.concat({
		messagePart,
		' Match IDs: ',
		table.concat(Array.sub(matchIds, 1, cutoff), ', '),
		#matchIds > cutoff and (', ... (' .. #matchIds .. ' total)') or '',
	})
