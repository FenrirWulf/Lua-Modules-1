...
party opponents, this fills in player's pageName using the displayName.
]]
...
function Opponent.readOpponentArgs(args)
	...
	if args.type == 'team' then
		return {type = 'team', template = args.template or args[1]}

	elseif args.type == 'literal' then
		return {type = 'literal', name = args.name or args[1]}

	elseif partySize == 1 then
		local player = {
			displayName = args[1] or args.p1 or args.name,
	...
	elseif partySize then
		local players = Array.map(Array.range(1, partySize), function(i)
			return {
				displayName = args[i] or args['p' .. i],
...
--[[
Creates an opponent struct from a match2opponent record.
